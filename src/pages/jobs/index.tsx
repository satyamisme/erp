...
export function BatchJobs() {
  ...
  useEffect(() => {
    fetchWithAuth('http://localhost:5000/api/dashboard/jobs')
      .then(res => res.json())
  ...
  return (
    <div className="p-6 flex-1 bg-surface-container-low">
      <div className="flex items-end justify-between mb-6">
        <div>
          <h2 className="text-headline-md text-2xl font-black text-primary tracking-tight">Active Operations <span className="text-slate-400 font-normal">/ Batch ID 326</span></h2>
          <p className="text-sm text-on-surface-variant">Monitoring asynchronous enterprise tasks for Lakki Quantum System.</p>
        </div>
        <div className="flex gap-2">
          <button className="flex items-center gap-2 px-4 py-2 bg-surface-container-lowest text-on-surface-variant border border-outline-variant/20 rounded-lg text-sm font-medium hover:bg-surface-container-high transition-colors">
            <span className="material-symbols-outlined text-sm">pause_circle</span> Pause Queue
          </button>
          <button className="flex items-center gap-2 px-4 py-2 bg-surface-container-lowest text-on-surface-variant border border-outline-variant/20 rounded-lg text-sm font-medium hover:bg-surface-container-high transition-colors">
            <span className="material-symbols-outlined text-sm">delete_sweep</span> Clear Completed
          </button>
          <button className="flex items-center gap-2 px-4 py-2 bg-error-container text-on-error-container rounded-lg text-sm font-bold hover:opacity-90 transition-opacity">
            <span className="material-symbols-outlined text-sm">cancel</span> Cancel All
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-surface-container-lowest p-4 rounded-xl border border-outline-variant/10 shadow-sm">
          ...
          <p className="text-[10px] text-secondary mt-2 flex items-center gap-1">
            <span className="material-symbols-outlined text-[12px]">trending_down</span> 0.01% from last hour
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        {data.activeJobs.map((job: any /* eslint-disable-line */, i: number) => (
          <div key={i} className="bg-white/60 backdrop-blur-md p-5 rounded-xl border border-outline-variant/20 shadow-sm">
            <div className="flex justify-between items-start mb-4">
              <div className={`p-2 rounded-lg ${i === 0 ? 'bg-primary/10 text-primary' : i === 1 ? 'bg-secondary/10 text-secondary' : 'bg-tertiary/10 text-tertiary'}`}>
                <span className="material-symbols-outlined">{i === 0 ? 'barcode_reader' : i === 1 ? 'print' : 'account_balance'}</span>
              </div>
              <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${i === 2 ? 'bg-tertiary-fixed text-on-tertiary-fixed-variant' : 'bg-secondary-container text-on-secondary-container'}`}>{i === 2 ? 'Parsing' : i === 1 ? 'Spooling' : 'Processing'}</span>
            </div>
      ...
      <div className="bg-surface-container-lowest rounded-xl border border-outline-variant/10 overflow-hidden">
        <div className="px-6 py-4 border-b border-surface-container-high flex justify-between items-center bg-white/50">
          <h3 className="text-sm font-bold text-primary">Job Queue History</h3>
          <div className="flex items-center gap-2">
            <div className="relative">
              <input type="text" placeholder="Search tasks..." className="bg-surface-container-low border-none rounded-lg text-xs py-1.5 pl-8 pr-4 w-64 focus:ring-1 focus:ring-primary" />
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-surface-container-high">
              {data.history.map((job: any /* eslint-disable-line */, i: number) => (
                <tr key={i} className="hover:bg-surface-container-low transition-colors group">
                  ...
                  <td className="px-6 py-3">
                    <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[10px] font-bold ${
                      job.status === 'SUCCESS' ? 'bg-secondary-container text-on-secondary-container' :
                      job.status === 'FAILED' ? 'bg-error-container text-on-error-container' :
                      job.status === 'RUNNING' ? 'bg-blue-100 text-blue-700' : 'bg-surface-container-high text-slate-500'
                    }`}>
                      <span className={`w-1.5 h-1.5 rounded-full ${
                        job.status === 'SUCCESS' ? 'bg-secondary' :
                        job.status === 'FAILED' ? 'bg-error' :
                        job.status === 'RUNNING' ? 'bg-blue-700 animate-pulse' : 'bg-slate-400'
                      }`}></span> {job.status}
                    </span>
                  </td>
